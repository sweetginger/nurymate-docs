...
| Shopify Markets 기능 | 누리메이트 채택 여부 | 이유 |
|--------------------|----------------|------|
...
| 서브폴더 URL 구조 | **채택 우선** | `nurymate.com/ja-jp`, `nurymate.com/ko-kr` 형식. 도메인 파워 집중 + 관리 단순화. ccTLD는 MAU 증가 후 검토 |
...
- 누리메이트는 **다수 셀러가 입점하는 마켓플레이스** → 마켓 컨텍스트가 플랫폼 전체 단위 + 개별 셀러 스토어 단위 두 레이어로 존재
