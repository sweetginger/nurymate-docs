...
**셀메이트CM는 누구나 30분 만에 쇼핑몰을 열고, 복잡한 설정 없이 글로벌 시장을 하나의 플랫폼으로 운영하는 차세대 헤드리스 이커머스 SaaS입니다.**
...
셀메이트CM는 단독 제품이 아니라 **셀메이트의 사업 범위 확장** 맥락에서 탄생했습니다.
...
```
구매자 → [셀메이트CM 판매 채널] → 주문 발생 → OMS → 셀메이트 ← 판매자
              ↑                                              ↑
         셀메이트의 새 접점                         기존 접점 유지
         (구매자 데이터·행동 확보)
```

셀메이트CM를 통해 셀메이트는 **판매 채널까지 사업 범위를 확장**함으로써:
...
이 때문에 셀메이트CM는 **스토어X처럼 소수의 특정 판매자를 타겟하는 버티컬 전략을 취하지 않습니다.** 스토어X는 깊은 버티컬 특화로 단가를 높이는 모델이지만, 판매 채널 사업은 판매자 수와 GMV 총량이 수익의 직접 변수이므로 범용 솔루션으로 매출 파이를 키우는 전략이 맞습니다.
...
카페24 대비 셀메이트CM만 보유한 3가지 구조적 차별점:
...
| **멀티테넌시 도메인** | `{셀러}.sellmatecm.com` 자동 생성 + 커스텀 도메인 연결 |
