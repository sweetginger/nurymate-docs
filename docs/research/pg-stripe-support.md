...
셀메이트CM이 국내결제(한국 PG)와 크로스보더 결제(Stripe)를 이원화하는 구조를 채택할 경우의 실현 가능성을 검토한다. 구체적으로는 Stripe의 한국 국내결제 지원 범위, 일본 결제수단 지원 현황, Stripe Connect 크로스보더 정산 구조, 그리고 이원화 구조 운영의 복잡도를 분석한다.
...
- 셀메이트CM이 직접 PG 연동을 구현할 경우 각 PG별 별도 SDK/API 연동 필요
...
4. **법인 구조가 핵심 제약**: 셀메이트CM이 Stripe를 활용하려면 해외 법인(일본 또는 싱가포르 등) 보유가 전제 조건이다.
