...
- 셀메이트CM이 PG 면허 없이도 Payoneer를 통해 글로벌 셀러 정산 가능한 법적 우회 경로 검토 가능
...
셀메이트CM이 Payoneer를 통해 마진을 수취하려면, 결제 수취 금액에서 수수료를 차감한 뒤 잔액만 Payoneer Payout API로 셀러에게 보내는 구조를 직접 구현해야 한다. (자동화 가능하나 Stripe Connect보다 구현 복잡도 높음)
