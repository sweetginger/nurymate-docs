...
누리메이트의 통계 고도화는 단순 매출 확인 수준의 통계 기능을 넘어, 머천트가 **데이터 기반 의사결정**을 직접 수행할 수 있는 분석 환경을 제공하는 것을 목표로 한다.
...
**카페24의 핵심 한계 (누리메이트가 반드시 해결해야 하는 항목):**
- 집계 주기 1시간 (실시간 불가)
- 코호트 분석 없음
- A/B 테스트 없음
- 퍼널 단계별 이탈율 분석 제한적
...
**누리메이트 목표: Shopify Advanced 수준을 기본 제공 (플랜 구분 없이)**
...
누리메이트 자체 추적 스크립트 또는 SDK를 통해 수집. GA4와 중복 수집하되, 누리메이트 자체 데이터를 Primary로 사용한다.
...
GA4는 세션 수준 행동 분석 및 광고 어트리뷰션의 보조 데이터 소스로 활용한다. 누리메이트 자체 트랜잭션 데이터와 GA4 데이터를 연결할 수 있도록 `client_id`를 주문 데이터에 함께 저장한다.
...
**GA4 데이터 접근 방식:**
- GA4 Data API (v1beta) 를 통해 주요 지표를 누리메이트 대시보드에 직접 임베드
- 머천트는 GA4 콘솔 접근 없이 누리메이트 어드민에서 GA4 데이터 확인 가능
...
- MMP 대시보드 데이터를 누리메이트 어드민 API로 pull
- 웹 + 앱 통합 고객 여정 가시화
...
- 방문자 집계: 누리메이트 자체 비콘 + GA4 Realtime API 병행
...
획득 월(cohort)별로 고객의 재구매율을 시각화한다. Shopify Standard 이상에서 제공하는 기능을 누리메이트는 기본 제공한다.
...
| 방식 | 장점 | 단점 | 적합도 |
|-----|------|------|------|
| **GA4 Data API** | GA4 데이터를 누리메이트 UI에 네이티브 통합 가능. 머천트가 GA4 콘솔 몰라도 됨 | API 할당량 제한(일 200k 요청). GA4 연결 설정 필요 | 높음 |
| **GA4 iframe 임베드** | 구현 간단. GA4 리포트 그대로 노출 | 누리메이트 디자인과 불일치. 머천트가 GA4 계정 필요. 커스터마이징 불가 | 낮음 |
...
[GA4]  ←→  [GA4 Data API]  →  [누리메이트 어드민 대시보드]
        ↑
...
| GA4 Data API | 채널 분석, 행동 데이터 Pull | 누리메이트 대시보드 통합 표시 |
| 누리메이트 DB | 모든 주문/결제/고객 데이터 | Primary Source of Truth |

**근거:**
1. GA4 Data API는 머천트가 GA4 콘솔을 별도로 학습할 필요 없이 누리메이트 어드민에서 모든 분석을 완결할 수 있게 한다
2. Measurement Protocol은 간편결제 리다이렉트 이후 발생하는 데이터 누락을 서버 사이드에서 완전 해소한다
3. 누리메이트 DB를 Primary로 유지함으로써 GA4 데이터 오차 발생 시에도 정확한 매출 수치를 확보한다

**GA4 연동 온보딩 (머천트 설정 최소화):**
1. GA4 측정 ID 입력 (1개 필드)
2. 누리메이트가 gtag.js 자동 삽입 + 권장 이벤트 자동 설정
...
> "통계가 틀리면 의사결정도 틀린다. 누리메이트는 간편결제까지 100% 추적되는 첫 번째 한국 이커머스 플랫폼이다."
