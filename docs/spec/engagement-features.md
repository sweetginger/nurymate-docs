...
> 작성일: 2026-03-20 | 우선순위: P2 | 관련 태스크: [19-2]
...
이커머스 플랫폼에서 신규 고객 유치 비용은 기존 고객 재방문 유도 비용의 **5~7배**에 달한다. 재방문율 향상 기능(게이미피케이션, 로열티 프로그램)은 엔드유저의 습관적 방문을 유도하고, 결과적으로 셀러의 GMV 성장과 셀메이트CM의 수수료 수익 증가로 이어진다.

셀메이트CM은 **헤드리스 커머스 구조**로 프론트엔드 구현이 다양하다. 셀러가 직접 개발한 스토어프론트이든, 셀메이트CM 기본 템플릿이든, 동일한 이벤트 인터페이스로 재방문율 향상 기능이 동작해야 한다.
...
| **식스샵** | ❌ 미지원 | 별도 출석체크 기능 없음. 적립금·쿠폰 자동지급은 구매완료/후기 조건으로만 지원 |
...
| **식스샵** | ✅ 기본 지원 | 구매, 가입, 후기 작성, 추천인 입력. 등급별 차등 적립률 설정 가능 | 유료 플랜 |
...
| **식스샵** | ✅ 기본 지원 | 회원 등급 생성·관리. 자동 등급 조정(구매금액 기준). 등급별 차등 적립률 설정. 등급별 추가 할인 혜택 설정 가능 |
...
셀메이트CM은 헤드리스 커머스 구조로, 프론트엔드 구현(Next.js, React Native, 외부 스토어프론트)에 무관하게 동일한 API 엔드포인트 호출만으로 로열티 기능이 동작해야 한다.
...
식스샵은 기본 로열티 기능(적립금·등급·쿠폰)을 네이티브로 지원하지만, 게이미피케이션 영역(출석체크·룰렛·스탬프·미션)은 전무하다.

**지원 기능**:
- **포인트(적립금)**: 구매금액 기반 적립. 가입·후기 작성·추천인 조건 자동지급. 등급별 차등 적립률 설정
- **회원 등급**: 구매금액 기준 자동 승급. 등급별 할인 혜택·적립률 차등. 카페24 수준의 등급 관리 지원
- **쿠폰 마케팅**: 조건별 쿠폰 자동지급(가입·구매완료·후기·추천인). 쿠폰 발급 시 카카오 알림톡/SMS/이메일 자동 발송으로 재방문 유도
- **고객 세그먼트**: 구매 이력·조건 기반 고객 그룹 생성 후 맞춤 메시지 발송 (빅인 연동으로 고도화 가능)
- **마케팅 자동화 연동**: 빅인(BigIn) 연동 시 고객 행동 분석 기반 자동 메시지 발송 가능

**미지원 기능 (카페24 대비 공백)**:
- 출석체크 (Daily Check-in) 없음
- 룰렛·스핀 이벤트 없음
- 스탬프 카드 없음
- 미션·퀘스트 없음
- 뱃지 시스템 없음

**시사점**: 식스샵은 카페24·아임웹과 비슷한 수준의 기본 로열티(포인트·등급·쿠폰)를 제공하지만, 재방문율에 직접 영향을 주는 게이미피케이션 기능 전반이 부재하다. 셀메이트CM이 출석체크·미션 등을 네이티브로 제공하면 식스샵 전환 셀러에게 명확한 차별화 포인트가 된다.
