...
> 작성일: 2026-03-31 | 우선순위: P2 | 관련 태스크: [19-2]
...
셀메이트CM는 **헤드리스 커머스 구조**로 프론트엔드 구현이 다양하다. 셀러가 직접 개발한 스토어프론트이든, 셀메이트CM 기본 템플릿이든, 동일한 이벤트 인터페이스로 재방문율 향상 기능이 동작해야 한다.
...
셀메이트CM는 헤드리스 커머스 구조로, 프론트엔드 구현(Next.js, React Native, 외부 스토어프론트)에 무관하게 동일한 API 엔드포인트 호출만으로 로열티 기능이 동작해야 한다.
...
**시사점**: 식스샵은 카페24·아임웹과 비슷한 수준의 기본 로열티(포인트·등급·쿠폰)를 제공하지만, 재방문율에 직접 영향을 주는 게이미피케이션 기능 전반이 부재하다. 셀메이트CM가 출석체크·미션 등을 네이티브로 제공하면 식스샵 전환 셀러에게 명확한 차별화 포인트가 된다.
